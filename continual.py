...
def create_base_model(
    ...
    split(DATA_FEATURIZED_PATH=DATA_FEATURIZED_PATH, DATA_SPLIT_PATH=DATA_SPLIT_PATH)
    ...
    move_replay_portion(
        dataset, DATA_COMBINED_PATH, Path("./replay_data/"), replay_portion / 100
    )
...
def continue_from_model(
    ...
    combine_replay_data(replay_datasets, DATA_COMBINED_PATH, replay_path)
    ...
    shutil.copy(
        METRICS_FILE_PATH,
        metrics_path / f"{len(replay_datasets)}-{dataset}-{dataset}.json",
    )

    # Prepare replay test data
    combine_test_data(replay_datasets, DATA_COMBINED_PATH, replay_path)

    # Evaluate on replay test data
    evaluate(
        MODELS_FILE_PATH=model_path / "model.h5",
        DATA_TRAIN_PATH=DATA_COMBINED_PATH / "train.npz",
        DATA_TEST_PATH=DATA_COMBINED_PATH / "test.npz",
        DATA_CALIBRATE_PATH=DATA_COMBINED_PATH / "calibrate.npz",
        INPUT_FEATURES_PATH=INPUT_FEATURES_PATH,
    ...
    shutil.copy(
        METRICS_FILE_PATH,
        metrics_path / f"{len(replay_datasets)}-{dataset}-combined_previous_data.json",
    )
    ...
    for ds in replay_datasets:
        ...
        shutil.copy(
            METRICS_FILE_PATH,
            metrics_path / f"{len(replay_datasets)}-{dataset}-{ds}.json",
        )
...
def run_continual_on_all_datasets_all_replays(
    basename: str,
    param_file: str,
    datasets: List[str],
    ...
    assert (
        min(replay_values) >= 0
    ), "All replay portions must be larger than 0 (percentage to replay)."
    assert (
        max(replay_values) <= 100
    ), "All replay portions must be smaller than 100 (percentage to replay)."

    move_param_file(param_file)

    # Run first time, creates original model which we use for other replay values
    run_continual_on_all_datasets(
        basename, datasets, replay_values[0], None, track_emissions
    )
    evaluate_basemodel_on_all(
        Path("./models", basename, str(replay_values[0])), datasets
    )

    basemodel = Path(
        "./models", basename, str(replay_values[0]), "models", "model_0.h5"
    )
    for replay_value in replay_values[1:]:
        ...
        run_continual_on_all_datasets(
            basename, datasets, replay_value, basemodel, track_emissions
        )
        evaluate_basemodel_on_all(
            Path("./models", basename, str(replay_value)), datasets
        )


def run_continual_on_all_datasets(
    basename: str,
    datasets: List[str],
    replay_value: int,
    basemodel: Path = None,
    ...
    # Starting and stopping emissions for each internal dataset, to be able to plot the trend in energy/emissions.

    if track_emissions:
        tracker = EmissionsTracker(
            project_name=f"{basename}_{replay_value}",
            save_to_file=True,
            log_level="critical",
    ...
    create_base_model(basename, datasets[0], replay_value, model_path, track_emissions)
    ...
    for dataset in datasets[1:]:
        ...
        continue_from_model(
            basename, dataset, replay_value, model_path, track_emissions
        )
...
def evaluate_basemodel_on_all(model_path: Path, datasets: List[str]):
    ...
    for dataset in datasets:
        ...
        evaluate(
            MODELS_FILE_PATH=model_path / "models" / "model_0.h5",
        ...
        for file in metrics_path.glob("*.json"):
            ...
            csv_file.write(
                f"model_version,trained_data,evaluated_data,{','.join(json_metrics.keys())}\n"
            )
            metrics.close()
            break
        ...
        for file in metrics_path.glob("*.json"):
            ...
            csv_file.write(
                f"{idx},{trained_data},{evaluated_data},{','.join([str(json_metrics[key]) for key in json_metrics.keys()])}\n"
            )
...
def baseline_metrics_to_csv(basedir: str):
    ...
    df = df.sort_values(
        ["model_version", "trained_data", "evaluated_data"],
...
def baseline_to_tex(name: str, dir: str):
    ...
    df.to_latex(
        ...
        columns=[
            ...
            if col not in ["model_version", "trained_data", "mse", "rmse" "mape"]
        ],
        float_format="%.3f",
    )


def combine_metrics_different_replay(
    gathered_filename: str,
    filename: str,
    exp_name: str,
    replays: List[int],
    metric: str,
    ...
    complete_df.to_latex(
        Path("./models", exp_name, gathered_filename), index=False, float_format="%.3f"
    )
...
def _sort_dataframe(path: Path):
    ...
    df.to_csv(
        metrics_path,
        index=False,
        columns=[col for col in df.columns if col not in ["mse", "rmse" "mape"]],
    )
...
def wanted_metrics(path, basedataset):
    ...
    included_columns = [
        col for col in train_eval_same.columns if col != "model_version"
    ]
    ...
    train_eval_same.to_csv(
        path / "same_train_eval.csv", index=False, columns=included_columns
    )
    eval_base_data.to_csv(
        path / "eval_base_data.csv", index=False, columns=included_columns
    )
    eval_combined_previous_data.to_csv(
        path / "eval_combined_previous_data.csv", index=False, columns=included_columns
    )


def move_replay_portion(
    dataset: str, source: Path, destination: Path, replay_portion: float
):
    np.random.seed(2022)
    try:
        ...
        np.savez(destination / "train" / f"{dataset}.npz", X=X_train, y=y_train)
        shutil.copy(source / "test.npz", destination / "test" / f"{dataset}.npz")
        shutil.copy(
            source / "calibrate.npz", destination / "calibrate" / f"{dataset}.npz"
        )
